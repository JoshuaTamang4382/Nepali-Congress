<?php include('header.php') ?>

<div id="niyamawali-page">
  <section class="common-banner-section small-height">
    <div class="custom-container">
      <div class="text-container">
        <div class="inner">
          <h5>नियम र नियमहरु</h5>
          <p>गृहपृष्ठ
            नियम र नियमहरु</p>
        </div>
      </div>
    </div>
  </section>

  <section class="online-library-section">
    <div class="custom-container">
      <div class="common-content-wrapper">
        <div class="common-content-item">
          <div class="book-post-wrapper">
            <div class="book-post-item">
              <div class="book-post-sub-item">
                <div class="image-container">
                  <img src="img/books/niyamwali.png" alt="">
                </div>
              </div>
              <div class=" book-post-sub-item">
                <div class="content">
                  <div class="text-container">

                    <h6 class="date">शुक्रबार ३०, आश्विन २०७७</h6>
                    <h6 class="title">नेपाली कांग्रेसको नियमावली २०७६</h6>
                    <h6 class="description">नेपाली कांग्रेसको नियमावली २०७६</h6>
                  </div>
                  <div class="view-social-container">
                    <ul>
                      <li><span><i class="fa-solid fa-eye"></i></span>
                        <h6>1195</h6>
                      </li>
                      <li><span><i class="fa fa-share-alt" aria-hidden="true"></i></span>
                        <h6>share on Facebook</h6>
                      </li>
                    </ul>
                  </div>
                  <div class="button-wrapper">
                    <ul>
                      <li><button class="common-btn">अधिक हेर्नुहोस्</button></li>
                      <li><button class="common-btn">डाउनलोड गर्नुहोस्</button></li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="common-content-item">
          <div class="common-notice-container">
            <div class="common-title">
              <h6 class="common-btn">प्रेस विज्ञप्ति</h6>
            </div>
            <div class="common-detail">
              <div class="notice-wrapper">
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र अमेरिकी रास्ट्रपति
                      जो बाइडेन जलवाय [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
              </div>
              <div class="read-more-btn">
                <button>
                  सबै हेर्नुहोस्
                </button>
              </div>
            </div>
          </div>
          <div class="common-notice-container">
            <div class="common-title">
              <h6 class="common-btn">कांग्रेसको कर्म
              </h6>
            </div>
            <div class="common-detail">
              <div class="notice-wrapper">
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र अमेरिकी रास्ट्रपति
                      जो बाइडेन जलवाय [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
                <div class="notice-item">
                  <div class="date-container">
                    <h6>मंगलबार १६, कार्तिक २०७८</h6>
                  </div>
                  <div class="notice">
                    <p>नेपाली कांग्रेसका सभापति एवं प्रधानमन्त्री श्री शेरबहादुर देउवा र भारतका प्रधानमन्त्री
                      श्री नरेन्द्र [...]</p>
                  </div>
                </div>
              </div>
              <div class="read-more-btn">
                <button>
                  सबै हेर्नुहोस्
                </button>
              </div>
            </div>
          </div>
          <div class="common-notice-container">
            <div class="common-title">
              <h6 class="common-btn">Social Media</h6>
            </div>
            <div class="common-detail">
              <div class="content">
                <iframe src="https://www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2Fnepalicongresshq%2F&tabs=timeline&width=500&height=500&small_header=false&adapt_container_width=true&hide_cover=false&show_facepile=true&appId" width="500" height="500" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>

              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="pagination-section">
    <div class="common-pagination">
      <a href="#">&laquo;</a>
      <a href="#" class="active">1</a>
      <a href="#">2</a>
      <a href="#">3</a>
      <a href="#">4</a>
      <a href="#">5</a>
      <a href="#">6</a>
      <a href="#">&raquo;</a>
    </div>
  </section>
</div>

<?php include('footer.php') ?>